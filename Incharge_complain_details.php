...
<?php
$con = mysqli_connect('localhost', 'root', 'Liam5641.', 'crime_portal');
...
$location = 'some_location'; // Replace 'some_location' with the actual location value

// Prepare the statement
$stmt = $con->prepare("SELECT p_name FROM police WHERE location = ?");
$stmt->bind_param("s", $location); // 's' represents the string type
...
while ($row = $result->fetch_assoc()) {
    ?>
    <option> <?php echo htmlspecialchars($row['p_name']); ?> </option>
    <?php
}
